...
namespace Global {
namespace I18n {
static thread_local ThreadReference* g_zoneRulesConstructor = nullptr;
...
{
    HandleScope scope(env, "InitI18nZoneRules");
    if (!scope.IsOpen()) {
        return nullptr;
    }
    ...
    if (status != napi_ok) {
        HILOG_ERROR_I18N("InitI18nZoneRules: Define class failed when InitI18nZoneRules.");
        return nullptr;
    }
    status = napi_set_named_property(env, exports, "ZoneRules", constructor);
    if (status != napi_ok) {
        HILOG_ERROR_I18N("InitI18nZoneRules: Set property ZoneRules failed.");
        return nullptr;
    }
    g_zoneRulesConstructor = ThreadReference::CreateInstance(env, constructor, 1);
    if (!g_zoneRulesConstructor) {
        HILOG_ERROR_I18N("InitI18nZoneRules: Failed to create reference at init.");
        return nullptr;
    }
    return exports;
}
...
napi_value ZoneRulesAddon::ZoneRulesConstructor(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok) {
        HILOG_ERROR_I18N("ZoneRulesConstructor: Get parameter info failed");
        return nullptr;
    }
    std::string tzId;
    napi_valuetype valueType = napi_valuetype::napi_undefined;
    if (argc < 1) {
        HILOG_ERROR_I18N("ZoneRulesConstructor: Param count less then required");
        return nullptr;
    }
    status = napi_typeof(env, argv[0], &valueType);
    if (status != napi_ok) {
        HILOG_ERROR_I18N("ZoneRulesConstructor: Get param type failed.");
        return nullptr;
    }
    if (valueType != napi_valuetype::napi_string) {
        HILOG_ERROR_I18N("ZoneRulesConstructor: Param type error.");
        return nullptr;
    }
    int32_t code = 0;
    tzId = VariableConvertor::GetString(env, argv[0], code);
    if (code != 0) {
        return nullptr;
    }
    ZoneRulesAddon* obj = new (std::nothrow) ZoneRulesAddon();
    if (obj == nullptr) {
        HILOG_ERROR_I18N("ZoneRulesConstructor: Create ZoneRulesAddon failed.");
        return nullptr;
    }
    status =
    ...
    if (status != napi_ok) {
        delete obj;
        HILOG_ERROR_I18N("ZoneRulesConstructor: warp ZoneRulesAddon failed.");
        return nullptr;
    }
...
napi_value ZoneRulesAddon::NextTransition(napi_env env, napi_callback_info info)
{
    ...
    napi_status status = napi_get_cb_info(env, info, &argc, argv, &thisVar, nullptr);
    if (status != napi_ok) {
        HILOG_ERROR_I18N("ZoneRulesAddon::NextTransition: Get parameter info failed");
        return nullptr;
    }
...
{
    if (!VariableConvertor::CheckNapiIsNull(env, argValue)) {
        HILOG_ERROR_I18N("ZoneRulesAddon::GetFirstParameter, parameter is null.");
        errorCode = -1;
        return 0;
    }
    napi_valuetype valueType = napi_valuetype::napi_undefined;
    ...
    if (status != napi_ok) {
        HILOG_ERROR_I18N("ZoneRulesAddon::GetFirstParameter, get parameter type failed.");
        errorCode = -1;
        return 0;
    }
    if (valueType != napi_valuetype::napi_number) {
        HILOG_ERROR_I18N("ZoneRulesAddon::GetFirstParameter, parameter type error.");
        errorCode = -1;
        return 0;
    }
    double date = 0;
    ...
    if (status != napi_ok) {
        HILOG_ERROR_I18N("ZoneRulesAddon::GetFirstParameter, Get parameter date failed");
        errorCode = -1;
        return 0;
    }
    return date;
...
napi_value ZoneRulesAddon::GetZoneRulesObject(napi_env env, std::string tzId)
{
    if (g_zoneRulesConstructor == nullptr) {
        HILOG_ERROR_I18N("ZoneRulesAddon::GetZoneRulesObject: g_zoneRulesConstructor is nullptr");
        return nullptr;
    }
    size_t argc = 1;
    napi_value argv[1] = { nullptr };
    ...
    if (status != napi_ok) {
        HILOG_ERROR_I18N("ErrorUtil::NapiThrow: create string failed");
        return nullptr;
    }
    argv[0] = param;
    napi_value constructor = nullptr;
    if (!g_zoneRulesConstructor->GetReferenceValue(env, &constructor)) {
