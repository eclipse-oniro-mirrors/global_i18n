...
napi_value HolidayManagerAddon::HolidayManagerConstructor(napi_env env, napi_callback_info info)
{
    ...
    int32_t code = 0;
    std::string path = GetString(env, argv[0], code);
    if (code) {
        return nullptr;
    }
...
std::string HolidayManagerAddon::GetString(napi_env &env, napi_value &value, int32_t &code)
{
    size_t len = 0;
...
napi_value HolidayManagerAddon::IsHoliday(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok || !obj || !obj->holidayManager_) {
        HiLog::Error(LABEL, "Get HolidayManager object failed");
        return nullptr;
    }
