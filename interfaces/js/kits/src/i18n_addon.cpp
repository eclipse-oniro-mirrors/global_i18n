...
napi_value I18nAddon::GetTransliteratorInstance(napi_env env, napi_callback_info info)
{
    ...
    status = napi_get_reference_value(env, *g_transConstructor, &constructor);
    if (status != napi_ok) {
...
napi_value I18nAddon::GetLineInstance(napi_env env, napi_callback_info info)
{
    ...
    status = napi_get_reference_value(env, *g_brkConstructor, &constructor);
    if (status != napi_ok) {
...
napi_value I18nAddon::GetIndexUtil(napi_env env, napi_callback_info info)
{
    ...
    napi_value constructor = nullptr;
    status = napi_get_reference_value(env, g_indexUtilConstructor, &constructor);
    if (status != napi_ok) {
