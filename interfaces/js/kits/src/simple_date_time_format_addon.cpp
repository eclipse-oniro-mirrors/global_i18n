...
namespace Global {
namespace I18n {
static thread_local napi_ref* g_SimpleDateTimeFormatConstructor = nullptr;
...
napi_value SimpleDateTimeFormatAddon::InitSimpleDateTimeFormat(napi_env env, napi_value exports)
{
    ...
    g_SimpleDateTimeFormatConstructor = new (std::nothrow) napi_ref;
    ...
    status = napi_create_reference(env, constructor, 1, g_SimpleDateTimeFormatConstructor);
    if (status != napi_ok) {
        HILOG_ERROR_I18N(
            "InitSimpleDateTimeFormat: Failed to create reference g_SimpleDateTimeFormatConstructor at init.");
        return nullptr;
    }
...
napi_value SimpleDateTimeFormatAddon::GetSimpleDateTimeFormatByPattern(napi_env env, napi_callback_info info)
{
    ...
    napi_value constructor = nullptr;
    napi_status status = napi_get_reference_value(env, *g_SimpleDateTimeFormatConstructor, &constructor);
    if (status != napi_ok) {
...
napi_value SimpleDateTimeFormatAddon::SimpleDateTimeFormatConstructor(napi_env env, napi_callback_info info)
{
    ...
    std::shared_ptr<LocaleInfo> localeInfo = (argc > SIZE_WITHOUR_LOCALE)?
        VariableConvertor::ParseIntlLocale(env, argv[SIZE_WITH_LOCALE - 1]) : nullptr;
    ...
    obj->simpleDateTimeFormat_ = std::make_unique<SimpleDateTimeFormat>(skeleton, localeInfo, isBestPattern, errCode);
