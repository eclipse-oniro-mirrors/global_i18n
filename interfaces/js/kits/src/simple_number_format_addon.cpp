...
namespace Global {
namespace I18n {
static thread_local napi_ref* g_SimpleNumberFormatConstructor = nullptr;
...
napi_value SimpleNumberFormatAddon::InitSimpleNumberFormat(napi_env env, napi_value exports)
{
    ...
    g_SimpleNumberFormatConstructor = new (std::nothrow) napi_ref;
    ...
    status = napi_create_reference(env, constructor, 1, g_SimpleNumberFormatConstructor);
    if (status != napi_ok) {
        HILOG_ERROR_I18N(
            "InitSimpleNumberFormat: Failed to create reference g_SimpleNumberFormatConstructor at init.");
        return nullptr;
    }
...
napi_value SimpleNumberFormatAddon::GetSimpleNumberFormatBySkeleton(napi_env env, napi_callback_info info)
{
    ...
    napi_value constructor = nullptr;
    status = napi_get_reference_value(env, *g_SimpleNumberFormatConstructor, &constructor);
    if (status != napi_ok) {
...
napi_value SimpleNumberFormatAddon::SimpleNumberFormatConstructor(napi_env env, napi_callback_info info)
{
    ...
    std::shared_ptr<LocaleInfo> localeInfo = (argc > 1)? VariableConvertor::ParseIntlLocale(env, argv[1]) : nullptr;
    ...
    obj->simpleNumberFormat_ = std::make_shared<SimpleNumberFormat>(skeleton, localeInfo, errCode);
