...
napi_value SystemLocaleManagerAddon::GetLanguageInfoArray(napi_env env, napi_callback_info info)
{
    ...
    bool isSuccess = VariableConvertor::GetStringArrayFromJsParam(env, argv[0], languageList);
    if (!isSuccess) {
        return nullptr;
    }
    SortOptions options;
...
napi_value SystemLocaleManagerAddon::GetCountryInfoArray(napi_env env, napi_callback_info info)
{
    ...
    bool isSuccess = VariableConvertor::GetStringArrayFromJsParam(env, argv[0], countryList);
    if (!isSuccess) {
        return nullptr;
    }
    SortOptions options;
