...
{
    HandleScope scope(env, "InitEntityRecognizer");
    if (!scope.IsOpen()) {
        return nullptr;
    }
...
napi_value EntityRecognizerAddon::constructor(napi_env env, napi_callback_info info)
{
    ...
    if (argc < 1) {
        localeStr = LocaleConfig::GetEffectiveLocale();
    } else {
    ...
    if (U_FAILURE(localeStatus)) {
        HILOG_ERROR_I18N("EntityRecognizerAddon::constructor: Create icu::Locale failed.");
        return nullptr;
    ...
    status = napi_wrap_s(env, thisVar, reinterpret_cast<void *>(obj.get()),
        EntityRecognizerAddon::Destructor, nullptr, &TYPE_TAG, nullptr);
    if (status != napi_ok) {
        return nullptr;
    }
