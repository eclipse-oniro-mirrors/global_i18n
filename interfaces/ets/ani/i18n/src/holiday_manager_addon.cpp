...
ani_object I18nHolidayManagerAddon::Create(ani_env *env, [[maybe_unused]] ani_object object, ani_string icsPath)
{
    ...
    static const char *className = "L@ohos/i18n/i18n/HolidayManager;";
...
ani_boolean I18nHolidayManagerAddon::IsHoliday(ani_env *env, ani_object object, ani_object date)
{
    ...
    if (isUndefined) {
        ...
        std::vector<int> list;
        list.push_back(VariableConverter::GetDateValue(env, date, "getFullYear"));
        list.push_back(VariableConverter::GetDateValue(env, date, "getMonth") + MONTH_INCREASE_ONE);
        list.push_back(VariableConverter::GetDateValue(env, date, "getDate"));
...
{
    static const char *className = "L@ohos/i18n/i18n/HolidayLocalNameInner;";
    ani_class cls;
...
{
    static const char *className = "L@ohos/i18n/i18n/HolidayInfoItemInner;";
    ani_class cls;
    if (ANI_OK != env->FindClass(className, &cls)) {
        HILOG_ERROR_I18N("Find class '%{public}s' failed", className);
        return nullptr;
...
ani_object I18nHolidayManagerAddon::GetHolidayInfoItemArray(ani_env *env, ani_object object, ani_object year)
{
    ...
    if (isUndefined) {
        ...
        ani_double yearValue;
        if (ANI_OK != env->Object_CallMethodByName_Double(year, "unboxed", ":D", &yearValue)) {
