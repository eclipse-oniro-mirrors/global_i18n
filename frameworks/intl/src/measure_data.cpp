...
const std::unordered_map<std::string, std::vector<double>> CONVERT_FACTORS {
    ...
    { "megabyte", { pow(10, 6), 0 } },
    { "gigabyte", { pow(10, 9), 0 } },
    { "terabyte", { pow(10, 12), 0 } },
    { "petabyte", { pow(10, 15), 0 } },
};
...
bool ConvertByte(double& number, std::string& unit)
{
    ...
    for (size_t i = 0; i < BYTE_VALUE.size(); i++) {
        if (BYTE_VALUE[i] == tempUnit) {
            indexFirst = static_cast<int>(i);
            break;
        }
    }
    bool status = (indexFirst >= 0);
    size_t indexSecond = status ? static_cast<size_t>(indexFirst) : 0;
    for (size_t i = indexSecond; i > 0; i--) {
        if (tempNumber < 1) {
