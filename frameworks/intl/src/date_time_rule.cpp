...
void DateTimeRule::LoadStrToPattern(std::unordered_map<std::string, icu::RegexPattern*>& map, xmlNodePtr cur)
{
    while (cur != nullptr) {
        ...
        if (value != nullptr && !xmlStrcmp(value->name, reinterpret_cast<const xmlChar*>("flag"))) {
            std::string type = reinterpret_cast<const char*>(xmlNodeGetContent(value));
            if (type == "True") {
                flag = true;
            }
            value = value->next;
        ...
        while (value != nullptr && !xmlStrcmp(value->name, reinterpret_cast<const xmlChar*>("content"))) {
            icu::UnicodeString tempContent = reinterpret_cast<char*>(xmlNodeGetContent(value));
            content += tempContent;
            value = value->next;
        }
...
void DateTimeRule::LoadStrToStr(std::unordered_map<std::string, std::string>* map, xmlNodePtr cur)
{
    while (cur != nullptr) {
        ...
        if (value != nullptr && !xmlStrcmp(value->name, reinterpret_cast<const xmlChar*>("level"))) {
            std::string levelStr = reinterpret_cast<const char*>(xmlNodeGetContent(value));
            int32_t status = 0;
        ...
        std::string content;
        while (value != nullptr && !xmlStrcmp(value->name, reinterpret_cast<const xmlChar*>("content"))) {
            std::string tempContent = reinterpret_cast<const char*>(xmlNodeGetContent(value));
            content += tempContent;
            value = value->next;
        }
        (*map)[key] = content;
...
// replace subStr in src with newStr.
std::string DateTimeRule::SubReplace(std::string& src, const std::string& subStr, const std::string& newStr)
{
    std::string::size_type pos = 0;
    std::string result = src;
    if (subStr.empty() || subStr.compare(newStr) == 0) {
        return result;
    }
    while ((pos = result.find(subStr)) != std::string::npos) {
        result.replace(pos, subStr.length(), newStr);
    }
    return result;
}
