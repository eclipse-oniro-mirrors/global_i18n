...
I18nErrorCode MultiUsers::LoadGlobalParam(const std::string& localId)
{
    ...
    std::string updatedLanguage = LocaleConfig::ComputeEffectiveLanguage(newLocale, newLanguage);
    UpdateConfiguration(updatedLanguage, newIs24Hour, userId);
...
#ifdef SUPPORT_GRAPHICS
void MultiUsers::UpdateConfiguration(const std::string& locale, const std::string& is24Hour,
    int32_t userId)
{
