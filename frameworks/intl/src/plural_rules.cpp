...
void PluralRules::ParseAllOptions(std::map<std::string, std::string> &options)
{
    ...
    localeMatcher = localeMatcher.empty() ? "best fit" : localeMatcher;
    type = ParseOption(options, "type");
    type = type.empty() ? "cardinal" : type;
    ...
    if (!minSignificantStr.empty() || !maxSignificantStr.empty()) {
        // 1 is minValue and defaultValue, 21 is maxValue
        ...
        if (!minFractionStr.empty() || !maxFractionStr.empty()) {
    ...
    std::map<std::string, std::string> &options)
{
    ...
    localeTags.push_back(LocaleConfig::GetEffectiveLocale());
    for (size_t i = 0; i < localeTags.size(); i++) {
        ...
        if (LocaleInfo::allValidLocales.count(locale.getLanguage()) > 0) {
            ...
            if (U_FAILURE(status) || !pluralRules) {
                continue;
            }
            createSuccess = true;
            break;
...
std::string PluralRules::Select(double number)
{
    ...
    if (U_FAILURE(status)) {
        HILOG_ERROR_I18N("PluralRules::Select: Format double failed.");
        return "";
    }
    icu::UnicodeString unicodeString = pluralRules->select(formattedNumber, status);
    if (U_FAILURE(status)) {
        HILOG_ERROR_I18N("PluralRules::Select: PluralRules select failed.");
        return "";
    }
    std::string result;
