...
private:
    std::string ValidateHolidayFilePath(const char* path);
    std::vector<HolidayInfoItem> ReadHolidayFile(const char* path);
    ...
    static const int32_t MONTH_GREATER_ONE = 1; // 1 is real month greater than display
    static const int32_t YEAR_START = 1900; // 1900 is where tm_year start
};
} // namespace I18n
} // namespace Global
} // namespace OHOS
