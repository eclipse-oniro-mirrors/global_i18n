...
class DateTimeRule {
public:
    DateTimeRule(std::string& locale);
    ~DateTimeRule();
    ...
    std::unordered_map<std::string, icu::RegexPattern*> GetPatternsMap();
    ...
    static std::string SubReplace(std::string& src, const std::string& subStr, const std::string& newStr);

private:
    void Init(std::string& locale);
    void RuleLevel();
    void InitRules(std::string& xmlPath);
